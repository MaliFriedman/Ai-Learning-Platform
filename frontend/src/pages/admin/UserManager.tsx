...
export default function UserManager() {
  ...
  const [formUser, setFormUser] = useState<Partial<User> & { id?: string }>({
    id: "",  
    name: "",
    phone: "",
  });
...
const handleFormSubmit = async () => {
  try {
    const cleanPhone = formUser.phone?.replace(/\D/g, "") || "";

    if (
      formUser.id &&
      users.some((u) => u._id === formUser.id)
    ) {
      const res = await axiosInstance.put(`/users/${formUser.id}`, {
        name: formUser.name,
        phone: cleanPhone,
      });
      setUsers((prev) =>
        prev.map((u) => (u._id === formUser.id ? res.data : u))
      );
    } else {
        ...
        >
          ...
          {isAdmin && (
            <>
              ...
              <button
                onClick={() => {
                  setFormUser({ id: user._id, name: user.name, phone: user.phone });
                  setShowForm(true);
                }}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-96">
            <h3 className="text-lg font-semibold mb-4">
              {formUser.id && users.some((u) => u._id === formUser.id)
                ? "Edit User"
                : "Add User"}
            </h3>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                value={formUser.id || ""}
                onChange={(e) =>
                  setFormUser((prev) => ({ ...prev, id: e.target.value }))
                }
